visPElNino <- function(yminmax = c(0,250),
                       x.text.pos = c(150, 750, 350, 1550, 900, 700),
                       y.text.pos = c(70, 200, 125, 60, 5, 170),
                       ...){

  precip$Index <- paste0(substr(precip$ts, 1, 4), c(rep("-1",2), rep("-2", 3), 
                                                    rep("-3", 4), rep("-4", 3)))
  ...
  colors <- c("black", "blue3", "red", "cornflowerblue", "green")
  #yminmax<- c(0, 250)
  #yminmax<- c(-150, 150)
  
  # 12 month season
  precip.seasonalwetdry <- seasonalMean(
    st = c(1973,1973,1992,2001), nd = c(2013,1991,2000,2013),
    st.shift = 7, nd.shift = 0, timespan = 12, fun = avrg, prm = "P_RT_NRT")

  precip.seasonalwetdry <- seasonalMean(
    st = c(1973,1973,1983,1993,2002), nd = c(2013,1982,1992,2002,2012),
    st.shift = 7, nd.shift = 0, timespan = 12, fun = avrg, prm = "P_RT_NRT")
  
  precip.seasonalwetdry.split <- 
    split(precip.seasonalwetdry, precip.seasonalwetdry$season)
  precip.seasonal.normal <- 
    list(lapply(precip.seasonalwetdry.split, function(x){x$p_dyn})$"1973-2013")
  
  plot.precip.seasonalwetdry.all <- 
    visSeasonPlotByAOI(
      lapply(precip.seasonalwetdry.split, function(x){x$p_dyn}), colors,
      linetype = c(2,1,1,1,1), ymin = yminmax[1], ymax = yminmax[2], timespan = 12,
      x.text = c(450,850,500,350, 100), 
      y.text = c(40,200,11,65, 100),
      labels.text = c("normal", "1973-1982", "1983-1992","1993-2002", "2003-2012"),
      colors.text = colors)
  
  aggregate(P_RT_NRT ~ substr(ts, 1, 3), data = precip, sum)
  ...
  plot.precip.20m.enso.elnino.all <- 
    visSeasonPlotByAOI(precip.20m.enso.split.median[1], color,
                       ...
                       colors.text = color)
  plot.precip.20m.enso.elnino.all
  print(plot.precip.20m.enso.elnino.all)
  ...
  plot.precip.20m.enso.elnino.pure <- 
    visSeasonPlotByAOI(precip.20m.enso.split.median[1], color,
                       ...
                       colors.text = color)
  plot.precip.20m.enso.elnino.pure
  print(plot.precip.20m.enso.elnino.pure)
  ...
  plot.precip.20m.enso.elnino.pure.ms <- 
    visSeasonPlotByAOI(precip.20m.enso.split.median[1], color,
                       ...
                       colors.text = color)
  plot.precip.20m.enso.elnino.pure.ms
  print(plot.precip.20m.enso.elnino.pure.ms)
  ...
  plot.precip.20m.enso.elnino.wIOD.all <- 
    visSeasonPlotByAOI(precip.20m.enso.split.median[1], color,
                       ...
                       colors.text = color)
  plot.precip.20m.enso.elnino.wIOD.all
  print(plot.precip.20m.enso.elnino.wIOD.all)
  ...
  plot.precip.20m.enso.elnino.wIOD.ms <- 
    visSeasonPlotByAOI(precip.20m.enso.split.median[1], color,
                       ...
                       colors.text = color)
  plot.precip.20m.enso.elnino.wIOD.ms
  print(plot.precip.20m.enso.elnino.wIOD.ms)
  ...
  plot.precip.20m.iod.p.all <- 
    visSeasonPlotByAOI(precip.20m.enso.split.median[1], color,
                       ...
                       colors.text = color)
  plot.precip.20m.iod.p.all
  print(plot.precip.20m.iod.p.all)
