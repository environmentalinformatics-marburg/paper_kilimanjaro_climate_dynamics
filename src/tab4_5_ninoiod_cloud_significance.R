...
# all El Nino - 3 years
cld.enid.all.nino <- subset(cld.enid, 
                            cld.enid$ENSO == "ME" |
                              cld.enid$ENSO == "SE" |
                              cld.enid$ENSO == "WE")
cld.enid.all.nino$type <- "all"
cld.enid.all.nino$phase <- 2
cld.enid.alles <- rbind(cld.enid.all, cld.enid.all.nino)

# pure El Nino - 2 years
...
cld <- melt(cld, id.vars = c("YEAR", "P_SQRT2"))
...
mod_glm <- glm(cld$value ~ sin_month_percent + 
                 cos_month_percent + as.factor(cld$variable))
summary(mod_glm)
...
# purest IOD+ (incl. weak El Nino & weak La Nina) - 1 years
